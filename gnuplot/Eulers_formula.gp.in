...
# set terminal pngcairo size 1280,1536
# set terminal pngcairo size 1280,1280
# set terminal pngcairo size 1024,1024    # Scaling factor = 80%
# set terminal pngcairo size 896,896      # Scaling factor = 70%
set terminal pngcairo size 832,832      # Scaling factor = 65%
# set terminal pngcairo size 768,768      # Scaling factor = 60%
# set terminal pngcairo size 1280,960
# set terminal pngcairo size 960,540
# set terminal pngcairo size 480,270
# set terminal pngcairo size 240,135
set output   filename
set print    "-"
...
if (plotAll != 0) \
	...
	for [x_new = 0:index] \
	\
	arrow from (x_new * plot_inc),0,0 to (x_new * plot_inc),0,cos((pi / 2) * x_new * plot_inc) linecolor "red"   nohead
...
if (plotAll != 0) \
	...
	for [x_new = 0:index] \
	\
	arrow from (x_new * plot_inc),0,0 to (x_new * plot_inc),sin((pi / 2) * x_new * plot_inc),0 linecolor "blue"  nohead

set \
\
for [x_new = 0:index] \
\
arrow from (x_new * plot_inc),0,0 to (x_new * plot_inc),sin((pi / 2) * x_new * plot_inc),cos((pi / 2) * x_new * plot_inc) linecolor "green" nohead
...
if (plotAll != 0) \
	...
	u,  sin((pi / 2) * u),  cos((pi / 2) * u)  linecolor "magenta"  linewidth 0.5 notitle; \
else \
	\
	splot \
	\
	u, sin((pi / 2) * u), cos((pi / 2) * u) linecolor "green" linewidth 0.5 notitle
